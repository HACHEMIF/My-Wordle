...
function wordle() {
    var motSecret = (0, exports.getRandomWord)(); // on crée une variable motSecret, dans lequel on va mettre un mot du tableau words[] de la function getRandomWord()
    var essai = 6; // l'utilisateur a 6 essais pour trouver le mot
    var jeuGagne = false; // de base, l'utilisateur n'a pas trouvé le mot, on instancie par faux. S'il trouve, on change pour true.
    var _loop_1 = function () {
        var motCherche = (0, exports.getInput)("Trouvez le mot secret. Vous avez " + essai + " tentative(s)"); // on demande à l'utilisateur de rentrer et tenter de deviner le mot. Et on met le mot rentré par l'utilisateur en majuscule, comme demandé
        var longueurMot = motCherche.length;
        var regex_lettre = /[^a-z]/gi;
        ...
        var motCMaj = motCherche.toUpperCase(); // on met notre mot en majuscule
        var coloredMot = ''; // variable dans laquel je mets les lettres qui seront bonnes ou pas, colorées
        var motTrouve = true; // on instancie true, mais des que l'utilisateur a une seule lettre fausse, on change pour false
        var tabMot = motCMaj.split('');
        tabMot.forEach(function (element, i) {
